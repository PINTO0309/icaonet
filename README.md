...
`output_reqs` value with its ICAO requirement id and description. Use
...
compliant and `score < 0.5` is treated as non-compliant. The script prints the
raw model value as `compliance_score`, prints `issue_score = 1 - score` for a
problem-oriented view, and prints `PASS` or `FAIL` using
...
Backend: onnx
[02] Blurred: compliance_score=0.200350285, issue_score=0.799649715, status=FAIL
[03] Looking away: compliance_score=0.977618456, issue_score=0.022381544, status=PASS
[04] Ink marked/creased: compliance_score=0.999709547, issue_score=0.000290453, status=PASS
...
```
